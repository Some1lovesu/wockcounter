    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://inspirobot.me/api?generate=true") as resp:
                if resp.status != 200:
                    await interaction.followup.send("❌ InspiroBot didn't feel inspired. Try again.", ephemeral=True)
                    return
                image_url = await resp.text()
    except Exception:
        await interaction.followup.send("❌ Couldn't reach InspiroBot. The universe is silent.", ephemeral=True)
        return

    embed = discord.Embed(color=0x7289da)
    embed.set_image(url=image_url.strip())
    embed.set_footer(text="Generated by InspiroBot • WockCounter")
    await interaction.followup.send(embed=embed)
